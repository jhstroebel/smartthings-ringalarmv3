...
// Needs Review
func getLockStatus(apiRequest public.Request) (events.APIGatewayProxyResponse, error) {
	log.Printf("LocationID %v", apiRequest.LocationID)
	...
	/*
		ringDeviceInfo.Body[i].General.v2.Name
		ringDeviceInfo.Body[i].General.v2.DeviceType == "lock"
		ringDeviceInfo.Body[i].General.v2.zid
		ringDeviceInfo.Body[i].Device.v1.locked
	*/
...
func setLockStatus(apiRequest public.Request, command string) (events.APIGatewayProxyResponse, error) {
	...
	_, err = wsutil.LockStatus(zID, command, connection)
	if err != nil {
		return sendResponse(public.ProcessError{http.StatusInternalServerError, http.StatusText(http.StatusInternalServerError)})
	}

	//Not sure what this does, may need to review
	return sendResponse(public.ModeChangeResponse{"Success"})
}
...
func Handler(request events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	//lock cases
	case "lockstatus":
		return getLockStatus(apiRequest)
	case "lock":
